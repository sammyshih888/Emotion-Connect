...
class ViewController: UIViewController {
    ...
    private func addTextNode(faceGeometry: ARSCNFaceGeometry) {
        ...
        textNode.position = SCNVector3(-0.1, 0.2, -0.5)
...
extension ViewController: ARSCNViewDelegate {
    ...
    func renderer(_ renderer: SCNSceneRenderer, nodeFor anchor: ARAnchor) -> SCNNode? {
        ...
        node.geometry?.firstMaterial?.diffuse.contents = UIColor.white.withAlphaComponent(0.0)//        node.geometry?.firstMaterial?.fillMode = .
...
//                print("identifier: \(topResult.identifier), confidence: \(topResult.confidence)")
                    ...
                    if firstResult.confidence > 0.92 {
                        ...
                        (self?.textNode?.geometry as? SCNText)?.string = ename
                    }
                }
            }])
    }
